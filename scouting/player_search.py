...
def tratar_valor(valor):
    ...
    if isinstance(valor, (int, float)) and pd.isna(valor):
        return 'Não disponível'
    return valor
...
for _, jogador in dados_filtrados.iterrows():
    with st.expander(f"{tratar_valor(jogador['player.name'])} ({tratar_valor(jogador['player.team.name'])})"):
        ...
        idade = tratar_valor(int((pd.Timestamp.now().timestamp() - jogador['player.dateOfBirthTimestamp']) // (365.25 * 24 * 3600)) if pd.notna(jogador['player.dateOfBirthTimestamp']) else 'Não disponível')
        st.write(f"País: {tratar_valor(jogador['player.country.name'])} | Idade: {idade} anos")
